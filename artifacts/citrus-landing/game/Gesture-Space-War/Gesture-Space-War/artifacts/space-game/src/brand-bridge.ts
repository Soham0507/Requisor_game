 ...
 * generic "Brand Name" placeholder (no logo) when no host ever sends a
 * theme, so the game never leaks the original Requisor AI demo branding.
...
const DEFAULT_THEME: BrandTheme = {
  ...
  heading: "Space Survivor",
  headingSize: 14,
  headingColor: "#88aaff",
  tagline: "HAND GESTURE EDITION",
  fontUrl: null,
};
...
function applyTheme(theme: Partial<BrandTheme>) {
  ...
  // An empty/missing brandName or heading (e.g. the customizer's form starts
  // blank) must fall back to the placeholder, not render as blank text.
